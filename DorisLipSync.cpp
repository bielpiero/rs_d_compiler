...
DorisLipSync::DorisLipSync (SerialPort* mc, int faceId) {
	...
	xmlLipSyncFullPath = XML_FILE_LIP_SYNC_PATH;
	xmlVisemesCodesFullPath = XML_FILE_VISEMES_CODES_PATH;
	this->mc = mc;
	this->faceId = faceId;
    ...
    theFile.open(xmlLipSyncFullPath.c_str());
    ...
    // Find our root node
    root_node = doc.first_node("gestures");

     for (xml_node<> * Gesto_node = root_node->first_node("gesture"); Gesto_node; Gesto_node = Gesto_node->next_sibling()){
        FaceGesture* gesture = new FaceGesture();
        gesture->setName(std::string(Gesto_node->first_attribute("name")->value()));
        gesture->setId(std::string(Gesto_node->first_attribute("id")->value()));
        gesture->setType(std::string(Gesto_node->first_attribute("type")->value()));

        for(xml_node<> * state_node = Gesto_node->first_node("frame"); state_node; state_node = state_node->next_sibling()){
        	FaceFrame* state = new FaceFrame();
        	state->setId(std::string(state_node->first_attribute("id")->value()));

	        for(xml_node<> * Motor_node = state_node->first_node("motor"); Motor_node; Motor_node = Motor_node->next_sibling()){ //we store all the variables for the motors in a matrix of strings
	       		FaceMotor* motor = new FaceMotor();

	       		motor->setCardId(faceId);
	            motor->setId(std::atoi(Motor_node->first_attribute("id")->value()));
	            motor->setPos(std::atoi(Motor_node->first_attribute("pos")->value()));
	            motor->setSpeed(std::atoi(Motor_node->first_attribute("speed")->value()));
	            motor->setAcceleration(std::atoi(Motor_node->first_attribute("speed")->value()));
...
std::string DorisLipSync::syllableToViseme(const std::string syllable){
	...
	std::ifstream dorisVisemes;
    dorisVisemes.open(xmlVisemesCodesFullPath.c_str());
	...
	for (xml_node<> * viseme_node = root_node->first_node("Viseme"); viseme_node and not found; viseme_node = viseme_node->next_sibling()){
		iterViseme = std::string(viseme_node->first_attribute("name")->value());
		...
		for (unsigned int i = 0; i < gestures->size(); i++){
	       if(viseme_id == gestures->at(i)->getId()){ //compares if the name of the gesture is the one we are looking for
	            // Iterate over the motors
	       		...
	       		for (unsigned int j = 0; j < gestures->at(i)->framesSize(); j++){
		           	for(unsigned int k = 0; k < gestures->at(i)->frameAt(j)->motorsSize(); k++){ //we store all the variables for the motors in a string matrix
