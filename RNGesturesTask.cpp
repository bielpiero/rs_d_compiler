...
RNGesturesTask::RNGesturesTask(SerialPort* mc, const char* name, const char* description){
	this->maestroController = mc;
     ...
     for (xml_node<> * Gesto_node = root_node->first_node("gesture"); Gesto_node; Gesto_node = Gesto_node->next_sibling()){
        ...
        for(xml_node<> * state_node = Gesto_node->first_node("frame"); state_node; state_node = state_node->next_sibling()){
	        ...
	        for(xml_node<> * Motor_node = state_node->first_node("motor"); Motor_node; Motor_node = Motor_node->next_sibling()){ //we store all the variables for the motors in a matrix of strings
	       		...
	       		if(strcmp(Motor_node->first_attribute("cardId")->value(), XML_FACE_LABEL) == 0){
	       			motor->setCardId(1);
	       		} else if(strcmp(Motor_node->first_attribute("cardId")->value(), XML_NECK_LABEL) == 0){
	       			motor->setCardId(0);
	       		}
	            ...
	            motor->setAcceleration(std::atoi(Motor_node->first_attribute("acceleration")->value()));
